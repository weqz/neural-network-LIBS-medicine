...
def oneLayerNN():
    ...
    # 1、建立数据占位符，x [None, 82], y_true [None, 2]
    ...
    with tf.variable_scope("layer_1"):
        # 随机初始化权重和偏置
        weight_1 = tf.Variable(tf.random_normal([82, 100], mean=0.0, stddev=1.0), name='w_1')
        bias_1 = tf.Variable(tf.constant(0.0, shape=[100]))
        y_1 = tf.matmul(x, weight_1) + bias_1

    # 2、建立一个全连接层网络 w[100, 2], b[2]
    with tf.variable_scope("fc"):
        # 随机初始化权重和偏置
        weight_fc = tf.Variable(tf.random_normal([100, 2], mean=0.0, stddev=1.0), name='w')
        bias_fc = tf.Variable(tf.constant(0.0, shape=[2]))
